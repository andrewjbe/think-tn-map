...
for(county in c(county_list)) {
  ...
  system(
    paste0("quarto render inst/county-summary-pdf/county-summary-pdf.qmd ",
           "-P county:", county)
  )
  ...
  system(
    paste0("quarto render inst/one-county-all-metrics/one-county-all-metrics.qmd ",
           "-P county:", county)
  )
  
  cli::cli_alert("Report rendered successfully! Saving as PDF...")
  
  pagedown::chrome_print(here("inst", "one-county-all-metrics", "one-county-all-metrics.html"),
...
for(metric in c(metric_list)) {
  ...
  system(
    paste0("quarto render inst/one-metric-all-counties/one-metric-all-counties.qmd ",
           "-P metric:'", metric, "'")
  )
